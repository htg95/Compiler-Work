...
//#pragma declarations
//long double pathDistanceMatrix[NUM_NODES][NUM_NODES];
...
int main()
{	
	long double **pathDistanceMatrix;
    int i, j, k, x, y;
	...
	int size=(NUM_NODES)*(NUM_NODES)*sizeof(long double);
	int _4_zeros=1<<28;
	printf ("_4_zeros %d  %p \n",_4_zeros, _4_zeros);
	
	pathDistanceMatrix=(long double **) malloc(sizeof(long double *)*(NUM_NODES));
	pathDistanceMatrix[0]=(long double*)memalign(_4_zeros,size);
	for(i=0;i<NUM_NODES;i++)
	{	
  ...
  /*  void init_array()
{
    ...
    for(i= 0; i < NUM_NODES; i++) {
        for(j= 0; j < NUM_NODES; j++) {
            long double ra = 1.0 + ((long double)NUM_NODES * rand() / (RAND_MAX + 1.0));
