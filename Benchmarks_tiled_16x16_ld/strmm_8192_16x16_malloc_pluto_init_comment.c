...
int main()
{
  ...
  //long double a[NMAX][NMAX], b[NMAX][NMAX], c[NMAX][NMAX];
  long double **a;
  long double **b;
  long double **c;
...
if (NMAX >= 2) {
  for (t1=0;t1<=floord(NMAX-1,16);t1++) {
    for (t2=0;t2<=floord(NMAX-1,16);t2++) {
      for (t3=0;t3<=min(floord(NMAX-2,16),t2);t3++) {
        for (t4=16*t1;t4<=min(NMAX-1,16*t1+15);t4++) {
          for (t5=max(16*t2,16*t3+1);t5<=min(NMAX-1,16*t2+15);t5++) {
            lbv=16*t3;
            ubv=min(16*t3+15,t5-1);
#pragma ivdep
