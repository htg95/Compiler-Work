...
if (M >= 1) {
  if (N >= 1) {
    for (t2=0;t2<=floord(M,8);t2++) {
      for (t3=t2;t3<=floord(M,8);t3++) {
        for (t4=0;t4<=floord(N,8);t4++) {
          for (t5=max(1,8*t2);t5<=min(M,8*t2+7);t5++) {
            for (t6=max(1,8*t4);t6<=min(N,8*t4+7);t6++) {
              lbv=max(8*t3,t5);
              ubv=min(M,8*t3+7);
#pragma ivdep
#pragma vector always
  ...
  for (t2=0;t2<=floord(M,8);t2++) {
    for (t3=t2;t3<=floord(M,8);t3++) {
      for (t4=max(1,8*t2);t4<=min(M,8*t2+7);t4++) {
        lbv=max(8*t3,t4);
        ubv=min(M,8*t3+7);
#pragma ivdep
#pragma vector always
