...
/* Start of CLooG code */
if (NMAX >= 1) {
  for (t1=0;t1<=floord(NMAX-1,8);t1++) {
    for (t2=t1;t2<=floord(NMAX-1,8);t2++) {
      for (t3=0;t3<=floord(NMAX-1,8);t3++) {
        for (t4=8*t1;t4<=min(NMAX-1,8*t1+7);t4++) {
          for (t5=8*t3;t5<=min(NMAX-1,8*t3+7);t5++) {
            lbv=max(8*t2,t4);
            ubv=min(NMAX-1,8*t2+7);
#pragma ivdep
