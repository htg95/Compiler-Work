...
if (NUM_NODES >= 1) {
  for (t1=0;t1<=NUM_NODES-1;t1++) {
    for (t2=0;t2<=floord(NUM_NODES-1,8);t2++) {
      for (t3=0;t3<=floord(NUM_NODES-1,8);t3++) {
        for (t4=8*t2;t4<=min(NUM_NODES-1,8*t2+7);t4++) {
          for (t5=8*t3;t5<=min(NUM_NODES-1,8*t3+7);t5++) {
