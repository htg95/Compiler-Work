...
if (N >= 2) {
  for (t1=0;t1<=floord(N-1,8);t1++) {
    for (t2=0;t2<=floord(N-2,8);t2++) {
      for (t3=t2;t3<=floord(N-1,8);t3++) {
        if (t2 == t3-1) {
          for (t4=8*t1;t4<=min(N-1,8*t1+7);t4++) {
            for (t5=8*t2;t5<=8*t2+6;t5++) {
              for (t6=8*t2+8;t6<=min(N-1,8*t2+15);t6++) {
                b[t4][t6] -= a[t5][t6] * b[t4][t5];;
              }
            }
            b[t4][(8*t2+7)] /= a[(8*t2+7)][(8*t2+7)];;
            for (t6=8*t2+8;t6<=min(N-1,8*t2+15);t6++) {
              b[t4][t6] -= a[(8*t2+7)][t6] * b[t4][(8*t2+7)];;
            }
          }
        }
        if (t2 == t3) {
          for (t4=8*t1;t4<=min(N-1,8*t1+7);t4++) {
            for (t5=8*t2;t5<=min(N-2,8*t2+6);t5++) {
              b[t4][t5] /= a[t5][t5];;
              for (t6=t5+1;t6<=min(N-1,8*t2+7);t6++) {
                b[t4][t6] -= a[t5][t6] * b[t4][t5];;
              }
            }
          }
        ...
        if (t2 <= t3-2) {
          for (t4=8*t1;t4<=min(N-1,8*t1+7);t4++) {
            for (t5=8*t2;t5<=8*t2+7;t5++) {
              for (t6=8*t3;t6<=min(N-1,8*t3+7);t6++) {
                b[t4][t6] -= a[t5][t6] * b[t4][t5];;
              }
            }
          }
